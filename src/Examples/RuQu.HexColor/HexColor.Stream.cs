...
namespace RuQu
{
    ...
    {
        protected override (byte red, byte green, byte blue) ContinueRead(ref ReadCharBuffer bufferState, ref IntState state)
        {
            var bytes = bufferState.Chars;
            if (bytes.Length > 7)
            {
    ...
    {
        protected override (byte red, byte green, byte blue) ContinueRead(ref ReadBuffer bufferState, ref IntState state)
        {
            var bytes = bufferState.Bytes;
            if (bytes.Length > 7)
            {
                throw new FormatException("Only 7 utf-8 chars");
            }
        ...
        protected override void HandleFirstBlock(ref ReadBuffer bufferState)
        {
